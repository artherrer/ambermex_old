...
import { View, Text, StyleSheet, Platform, Image, Modal, Keyboard, ScrollView, Dimensions, TouchableOpacity, PixelRatio,  ActivityIndicator, TouchableWithoutFeedback,KeyboardAvoidingView, StatusBar, ActionSheetIOS } from 'react-native'
import { Icon,Slider, Input, Button as ButtonAlt } from 'react-native-elements';
...
class AlertModal extends Component{
  ...
  componentDidUpdate(prevProps, nextProps) {
    let { triggerAlertModal } = this.state;

    if((prevProps.confirmAlert && triggerAlertModal) && this.props.confirmAlert == false){
      setTimeout(async function(){
        this.reportAfterAlert();
      }.bind(this),1000);
    }
  }
  ...
  checkComplete(){
    ...
    if(sliderValue === 100){
      if(alertType === "Emergency"){
        ...
        this.setState({alertType:"Feminist", uploading:true, alertMessage:"Alerta Mujeres Activada"});
        setTimeout(function(){
          this.props.startAlert({alertType:"Feminist", alertMessage:"Alerta Mujeres Activada"});
        }.bind(this),200);
      }
      else if(alertType === "Medical"){
        this.setState({alertType:"Medical", uploading:true, alertMessage:"Alerta Medica Activada"});
        setTimeout(function(){
          this.props.startAlert({alertType:"Medical", alertMessage:"Alerta Medica Activada"});
        }.bind(this),200);
      }
      else{
        this.setState({alertType:"Fire", uploading:true, alertMessage:"Alerta de Incendio Activada"});
        setTimeout(function(){
          this.props.startAlert({alertType:"Fire", alertMessage:"Alerta de Incendio Activada"});
        }.bind(this),200);
      }
    }
    else{
  ...
  loadPictures(){
     ImagePicker.openPicker({
       ...
       if(image != null){
         ...
         setTimeout(function(){
           this.setState({reportImage:pic, padHeight:0});
         }.bind(this),200);
       }
       else{
 ...
 takePicture(){
   ...
   ImagePicker.openCamera({
    ...
    if(image != null){
      ...
      setTimeout(function(){
        this.setState({reportImage:pic, fileUploadModal:true });
      }.bind(this),200);
    }
    else{
      console.log('cancelled');
 ...
 getProviderInfo(alertType, fromOfficialChannel){
   if(this.props.UserData.membershipProducts != undefined && this.props.UserData.membershipProducts.find(o => o.type === 4)){
 ...
 getFontSize(originalSize){
  ...
  getMainComponent(alertType, fromOfficialChannel){
    if(alertType === "Emergency" && fromOfficialChannel){
      return <View style={{height:(height - headerHeight),alignItems:'center',justifyContent:'center',paddingTop:10,paddingBottom:10}}>
                ...
                <Text style={{color:'#7d9d78', marginBottom:0,fontWeight:'700',fontSize:this.getFontSize(35),marginTop:10, letterSpacing:-.5, textAlign:'center'}}>ALERTA VECINAL</Text>
                </View>
                ...
                {this.props.loading ?
                  <Text style={{alignSelf:'center',color:'#858585',fontSize:this.getFontSize(25),width:width/2, letterSpacing:-.5, textAlign:'center'}}>Procesando alerta</Text>
                  :
                  <View style={{width:'90%', alignItems:'center', justifyContent:'center', alignSelf:'center'}}>
                  <Text style={{textAlign:'center',fontSize:this.getFontSize(20), alignSelf:'center'}}>Solicitando apoyo para:</Text>
                  <Text numberOfLines={2} style={{marginTop:5,textAlign:'center', fontSize:this.getFontSize(17), fontWeight:'700'}}>{this.getAddressString()}</Text>
                  </View>
                }
                </View>
                <View style={{height:(((height - headerHeight)/5)), width:width, justifyContent:'flex-start'}}>
                <View style={{height:50, justifyContent:'center'}}>
                <Text style={{color:'white',fontSize:this.getFontSize(17), fontWeight:'700',textAlign:'center',position:'absolute',alignSelf:'center', zIndex:100}}>    DESLIZA PARA CONFIRMAR</Text>
      ...
      return  <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()} style={{height:(height - headerHeight), width:width,alignItems:'center',justifyContent:'center'}}>
                ...
                {this.getProviderInfo(alertType, fromOfficialChannel)}
                <Text style={{color:'#fcaf00',maxWidth:'80%', marginBottom:0,fontWeight:'700',fontSize:this.getFontSize(iPhoneX ? 28 : 24),marginTop:10, letterSpacing:-.5, textAlign:'center'}}>ACTIVIDAD SOSPECHOSA</Text>
                </View>
                {this.props.loading || this.state.uploading ?
                  <View style={{height:(((height - headerHeight)/5)*3), width:width}}>
                    ...
                    <Text style={{alignSelf:'center',color:'#858585',fontSize:this.getFontSize(22),width:width/2, letterSpacing:-.5, textAlign:'center'}}>Procesando reporte</Text>
                    </View>
                  </View>
                  :
                  <View style={{height:(((height - headerHeight)/5)*3), width:width}}>
                    <View style={{height:'60%'}}>
                      <Text style={{fontSize:this.getFontSize(15), left:'10%', marginBottom:10}}>Agrega una foto*</Text>
      ...
      return  <View style={{height:(height - headerHeight),alignItems:'center',justifyContent:'center',paddingTop:10,paddingBottom:10}}>
                <View style={{height:(((height - headerHeight)/5)*1.5), justifyContent:'flex-end'}}>
                {this.getProviderInfo(alertType, fromOfficialChannel)}
                <Text style={{color:'#0C479D', marginBottom:0,fontWeight:'700',fontSize:this.getFontSize(35),marginTop:10, letterSpacing:-.5, textAlign:'center'}}>ALERTA MÉDICA</Text>
                </View>
                <View style={{height:(((height - headerHeight)/5)*1.5), width:width, justifyContent:'center'}}>
                <View testID='LocationAlert' disabled={this.props.uploading || this.props.loadLocation} style={{height:(width/3.25), backgroundColor:'transparent', borderRadius:(width/3.25)/2, width:width/3.25, borderWidth:0, alignSelf:'center',justifyContent:'center'}}
                onPress={() => {
                ...
                {this.props.loading ?
                  ...
                  :
                  <Image source={MEDICA} style={{height:(width/3.25), width:(width/3), alignSelf:'center'}} resizeMode="contain" />
                }
                </View>
                </View>
                </View>
      ...
      return <View style={{height:(height - headerHeight),alignItems:'center',justifyContent:'center',paddingTop:10,paddingBottom:10}}>
                <View style={{height:(((height - headerHeight)/5)*1.5), justifyContent:'flex-end'}}>
                {this.getProviderInfo(alertType, fromOfficialChannel)}
                <Text style={{color:'#e30613', marginBottom:0,fontWeight:'700',fontSize:this.getFontSize(35),marginTop:10, letterSpacing:-.5, textAlign:'center'}}>ALERTA PERSONAL</Text>
                </View>
                <View style={{height:(((height - headerHeight)/5)*1.5), width:width, justifyContent:'center'}}>
                <View testID='LocationAlert' disabled={this.props.uploading || this.props.loadLocation} style={{marginTop:30,height:(width/3.25), backgroundColor:'transparent', borderRadius:(width/3.25)/2, width:width/3.25, borderWidth:0, borderColor:'white', alignSelf:'center', justifyContent:'center'}}
                onPress={() => {
  ...
  render(){
    let { alertType, officialChannel } = this.props;
    ...
    return (
      ...
      <View style={{
        ...
        <View style={{height:50,width:width,justifyContent:'flex-end', backgroundColor:"#f6f6f6", marginBottom:5}}>
          <Image source={BOTON_AMBERMEX} resizeMode={"contain"} style={{height:40, width:width/3, alignSelf:'center'}} />
        </View>
        <View style={{borderBottomWidth:1, borderColor:"#ddd",shadowColor: "#000",shadowOffset: {width: 0,height: 2,},shadowOpacity: 0.8,shadowRadius: 10,elevation: 3}} />
        <View style={{padding:10, alignItems:'flex-end',justifyContent:'flex-end', height:50,backgroundColor:'#f6f6f6'}}>
          ...
          this.setState({modal:false,picture:null, pictureModal:false, alertMessage:"", alertValid:true,padHeight:70, modalAlertHeight:iPhoneX ? (height - 350) : (height - 180)})}
            ...
            <Text style={{fontSize:20,color:'#e30613',letterSpacing:-.5}}>Cancelar</Text>
          </TouchableOpacity>
        </View>
        <View style={{alignItems:'center', width:width, borderBottomColor:'lightgray', borderBottomWidth:4, justifyContent:'center',backgroundColor:'#f6f6f6',height:(height/2) - (header)}}>
        <Text style={{color:'white',fontWeight:'bold',fontSize:30,marginBottom:20, color:"#e30613", width:width/2, textAlign:'center'}}>ALERTA SEGURIDAD</Text>
        <TouchableOpacity testID='LocationAlert' disabled={this.state.uploading || this.props.loadLocation} style={{height:(width/2.75), backgroundColor:this.state.loading ? 'gray' : "#e30613", borderRadius:(width/2.75)/2, width:width/2.75, borderColor:'black', borderWidth:0, alignSelf:'center',shadowOffset: {width: 0,height: 15},shadowOpacity:.5,shadowRadius:15, justifyContent:'center'}}
        onPress={() => {
          this.setState({alertType:"Emergency", alertMessage:"Alerta Activada", triggerAlertModal:true});
        }}>
        <View style={{height:'100%', width:'100%', paddingTop:2, borderRadius:100, backgroundColor:this.props.loading ? 'gray' : "#e30613", alignSelf:'center', justifyContent:'center', borderWidth:10, borderColor:'white'}}>
        <Image source={SEGURIDAD} style={{height:(width/2.5)/2, width:(width/2.5)/2, alignSelf:'center', top:-7}} resizeMode="contain" />
        </View>
        </TouchableOpacity>
        </View>
        <View style={{alignItems:'center',justifyContent:'center',backgroundColor:'#f6f6f6',height:(height/2) - (header)}}>
       <Text style={{color:'white',fontWeight:'bold',fontSize:30,marginBottom:20, color:"#f05a23", width:width/2.25, textAlign:'center'}}>ALERTA INCENDIO</Text>
       <TouchableOpacity testID='CreateAlert' disabled={this.state.loading} style={{height:(width/2.75),backgroundColor:this.state.loading ? 'gray' : "#f05a23", borderRadius:(width/2.75)/2, width:(width/2.75), borderColor:'black', borderWidth:0, alignSelf:'center', justifyContent:'center',shadowOffset: {width: 0,height: 15},shadowOpacity:.5,shadowRadius:15}}
       onPress={() =>
         {
           Keyboard.dismiss;
           this.setState({alertType:"FireAlert",alertMessage:"Alerta de Incendio Activada", triggerAlertModal:true});
         }}>
         <View style={{height:'100%', width:'100%', borderRadius:100, backgroundColor:'#f05a23', alignSelf:'center', justifyContent:'center', borderWidth:10, borderColor:'white'}}>
          <Icon type="material-community" name="fire" size={80} color="white" />
         </View>
         </TouchableOpacity>
       </View>
        </View>
          ...
          backdropPressToClose={false}
          backdrop={false}
          visible={this.props.confirmAlert}
          onRequestClose={() => {
            this.setState({triggerAlertModal:false});
            this.props.closeModal();
          }}>
            <ScrollView style={{flex:1, width:width, backgroundColor:'white'}}>
            ...
            <TouchableOpacity
                        onPress={() => {
                          this.setState({triggerAlertModal:false});
                          this.props.closeModal();}}  style={{height:50, marginLeft:5, width:40, justifyContent:'center'}}>
            ...
            { this.getMainComponent(alertType, officialChannel) }
            </View>
            </View>
            </ScrollView>
            </Modal>
